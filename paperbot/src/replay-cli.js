...
import { loadObservations, replaySweep, replayWarnings } from './replay.js';
...
const corpusPath = process.env.PAPER_REPLAY_CORPUS
  || join(process.cwd(), config.dataDirectory, 'orderbook-observations.jsonl');

const text = await readFile(corpusPath, 'utf8').catch(() => {
  throw new Error(`No observation corpus at ${corpusPath}. Run "npm run scan" first.`);
});

const observations = loadObservations(text);
